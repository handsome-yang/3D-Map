...
/**
 * 保留小数，默认保留两位小数
 * @param  {[type]} number     [需要保留小数的数字或者字符串]
 * @param  {[type]} demicalNum [需要保留的小数位数]
 * @return {[type]}            [description]
 */
function retainDemical(thisValue, demicalNum) {
    demicalNum = demicalNum || 2;
    if(thisValue != null && thisValue!= undefined && !isNaN(thisValue) && (thisValue.toString().split(".").length == 2)) { //是数字并且是小数
        return parseFloat(thisValue).toFixed(demicalNum);
    } else {
        return thisValue;
    }
}
 ...
 * TODO IE兼容性问题需要测试
 * 打开子窗口
 * @param  {[string]} url    [子窗口地址]
 * @param  {[object]} params [父窗口传给子窗口的参数]
 * @param  {[num]} width  [子窗口宽度]
 * @param  {[num]} height [子窗口高度]
 * @return {[object]}        [子窗口传给父窗口的参数]
 */
...
function setGridScrollHeight() {
    ...
    if($("#resultDivScroll").length == 0 || $("#dgDiv").length == 0){
        return;
    }
...
function setDivHeight() {
    ...
    if($("#resultDiv").length == 0 || $("#dgDiv").length == 0){
        return;
    }
    var outerHeight = top.STAMP_config.height.bannerHeight + top.STAMP_config.height.toolHeight;
    var minusHeight = outerHeight + $(".cardTitle").height() + $("#southDiv").height() + $("#northDiv").height();
 ...
 * 调整显示小分页控件
 */
function showSmallPagination(pageId) {
    var pageNextArr = $("#" + pageId + " td");
    pageNextArr[0].style.display = "none";
    pageNextArr[pageNextArr.length - 1].style.display = "none";
    $("#" + pageId + " .l-btn-left").css("padding-left", "0px");
    $("#" + pageId + " .l-btn-left").parent().css("padding-right", "0px");
    $("#" + pageId + " .l-btn-right").css("padding-left", "0px");
    $("#" + pageId + " .l-btn-right").parent().css("padding-left", "0px");
    $("#" + pageId + " .pagination-num").css("margin-left", "0px");
    $("#" + pageId + " .pagination-num").parent().prev().hide();
    $("#" + pageId + " .pagination-num").parent().next().children().css("padding-right", "0px");
    $("#" + pageId + " .pagination-num").parent().next().width(33);
    $("#" + pageId + " .pagination-num").parent().next().children().width(33);
    $("#" + pageId + " .pagination-num").removeAttr("size");
    $("#" + pageId + " .pagination-num").parent().width(25);
    $("#" + pageId + " .pagination-num").width(25);
    $("#" + pageId + " .pagination-num").parent().css("text-align", "center");
    $("#" + pageId + " .pagination-num").parent().next().css("text-align", "center");
    $("#" + pageId + " table").css("margin-left", "0px");
    $("#" + pageId + " table").css("margin-right", "0px");
}
