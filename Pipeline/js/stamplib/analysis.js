...
/**
 * 横断面分析画线完毕回调事件
 * @param {[object]} pObj [返回的线矢量对象]
 */
var OnCreateHorizonSectionLine = function (pObj) {
    if (!pObj || pObj.Count == 0) {
        return;
    }
    var thisImg = $("#AnalysisTranSection").find("img");//给横断面图标换成Loading并且禁用其他按钮
    ...
    var pipeLineLayers = top.LayerManagement.getPipeListByLayerChecked(layer);
    addServices(pipeLineLayers, pObj);
};
/**
 * 根据编码，获取编码对应的详细值
 * @param {[string]} type   [管线编码类型]
...
/**
 * 构造请求数组
 * @param {[Array]} layers [需要分析的图层对象集合]
 * @param {[object]} pObj   [横断面绘制的线矢量对象]
 */
...
/**
 * 发送请求解析结果
 * @param  {[array]} urlList [description]
 */
...
function createTranSectionChat() {
    ...
    disableAll(false);
    thisImg.attr("src", "images/tools/AnalysisTranSection.png");
    if (hasSvg()) {
        openDialog("html/analysis/tranSectionResult.html", params, 1060, 700);
    } else {
        openDialog("html/analysis/tranSectionResult2.html", params, 1060, 700);
    }
}
/**
 * 获取绘制的线的点坐标字符串
 * @param  {[object]} pObj [线矢量对象]
 * @return {[string]}      [坐标字符串]
 */
...
var maxGroundAltitude = 0;//最大地形搞笑
...
/**
 * 解析结果
 * @param  {[string]} xmlDoc      [xml字符串]
 * @param  {[string]} layerId     [该管线图层的guid]
 * @param  {[string]} type        [图层类型,编码]
 * @param  {[string]} customColor [自定义的颜色]
...
function parseResult(xmlDoc, layerId, type, customColor) {
    ...
    for (var i = 0; i < count; i++) {
        ...
        var dataType = getValueByCode("PipeCode", type); //数据类型
 ...
 * 返回值：无
 */
function analysisSelectObj(contain, btn, bufferDiskObj, pointCheck, objAnalys, isNeedRadius) {
    ...
    top.LayerManagement.earth.Event.OnPickObjectEx = function (pObj) {
        ...
        obj.Z = pObj.SphericalTransform.Altitude; //top.LayerManagement.earth.Measure.MeasureTerrainAltitude(obj.X, obj.Y);
        var lineChk = true;
        ...
        if (lineChk) {
            var checkFlag = checkProject(pObj, isNeedRadius);
            if (!checkFlag) {
        ...
        selobj = pObj;
        selectCenter = pObj.GetLonLatRect().Center;
    };
    top.LayerManagement.earth.Query.PickObjectEx(24); // SEPickObjectType.PickAllObject
}
/**
 * 判断分析的管线是否属于当前工程
 * @param  {[object]}  pObj         [管线或者管点模型对象]
 * @param  {Boolean} isNeedRadius [是否需要生成缓冲面]
 * @return {[type]}               [true:属于,false:不属于]
 */
function checkProject(pObj, isNeedRadius) {
    ...
    if (parentLayerName.indexOf("container") > -1) { // line  RecordCount = 0
        ...
        if (result.RecordCount > 0) {
            if (isNeedRadius) {
                var page0 = result.GotoPage(0);
                if (page0) {
                    page0 = $.xml2json(page0);
                    if (page0) {
                        var pageResult = page0.Result;
                        if (pageResult) {
                            var pipeLength = pageResult.Geolength;
                            if (pipeLength) {
                                return pipeLength / 2;
                            } else {
                                return 0.1;
...
/**
 * 爆管分析选取按钮点击事件
 * @param  {[object]} contain    [jquery对象，需要保存管线编号的input框]
 * @param  {[object]} btn        [jquery对象，分析按钮]
 * @param  {[number]} bufferDisk [选取管线需要生产的缓冲的半径]
 * @param  {[string]} pointCheck [是否分析点]
 * @param  {[type]} objAnalys  [是否需要在进行分析]
 * @return {[type]}            [description]
 */
 ...
 * @param  {[string]} type      [管线或者管点]
 * @param  {[string]} key       [要高亮显示的关键字集合，以逗号拼接]
 * @param  {[Boolean]} checkFlag [true：显示高亮；false：取消高亮]
 * @param  {[string]} lineType  [如果有,则是管线只有这种子图层下会高亮]
 * @param  {[string]} pointType [如果有,则是管点只有这种子图层下会高亮]
 * @return {[type]}           [description]
 */
    ...
    for (i = 0; i < layer.GetChildCount(); i++) {
        try {
            ...
            if (checkFlag) {
                ...
                dbClickHighLight = [];//防止下一次双击清除上一次结果时崩溃的问题
                selobj = null;
            }
 ...
 * 获取坐标
 * @param  {[strIng]} layerID [图层guid]
 * @param  {[string]} data    [服务返回的xml字符串]
 * @param  {[string]} usKey   [关键字]
 * @return {[obejct]}         [坐标对象]
 */
...
/**
 * 坐标转换
 * @param  {[Array]} coord   [经纬度坐标数组]
 * @return {[type]}         [经纬度坐标和平面坐标对象]
 */
 ...
 * 得到没有模型的点模型的详细信息
 * @param  {[string]} layerID [图层guid]
 * @param  {[Obejct]} record  [当前记录]
 * @param  {[obejct]} obj     [模型对象]
 */
