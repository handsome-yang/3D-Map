...
var MarkedCreator = {
    ...
    /**
     * 判断是否创建相同的标注
     * @param  {[string]} tag [标注名称]
     * @param  {[Boolean]} isCheck   [是否需要验证]
     * @return {[Boolean]}     [true:重名,false:不重名]
     */
    getMarkedFileByTag: function (tag, isCheck) {
        if (!isCheck) {
    ...
    /**
     * 保存标注文档信息
     * @param  {[string]} markedXml [要保存的标注文档信息]
     */
    ...
    /**
     * 添加OnPickObjectEx监听事件，选取一条管线
     * @param {[number]} queryTableType  [表查询类型（1为线表查询，0为点表查询）]
    ...
    /**
     * 创建标注对象
     * @param {[object]}  dataParam [创建标注对象所需的参数]
     * @return [标注对象]
     */
    createElementIcon: function (dataParam, isCheck) {
        ...
        //检查此标注是否已经创建过
        if (this.getMarkedFileByTag(dataParam.tag, isCheck)) {
            return false;
        }
        ...
        markedIcon.ShowHandle = showHandle;//是否显示标注竖线
        markedIcon.HandleHeight = handleHeight;//标注竖线高度
        markedIcon.TextFormat = 256;
        markedIcon.minVisibleRange = 0;
        markedIcon.maxVisibleRange = 100;//最大可视高度
    ...
    initMarkedIconList: function () {
        ...
        for (var i = 0; i < elemMarkedList.length; i++) {
            ...
            if (lineCoordNode != null) { //判断标注节点是否为距离标注节点，如果是距离标注节点，则创建距离标注
                ...
                dataParam.p1 = p1;
                dataParam.p2 = p2;
                if (elemMarked.parentNode.tagName == "MarkedComplex") { //判断是否为扯旗标注
                    ...
                    this.createElementIcon(dataParam, true); //创建标注对象
                }
            } else { //否则创建其它标注
                var text = elemMarked.selectSingleNode("Text").text;
                text = text.replace(/\EN/g, '\n'); //将所有\EN转换为回车符
        ...
        for (var i = 0; i < this.markedIconList.length; i++) {
            if (this.markedIconList[i].Guid == id) {
                ...
                if (childMarkeds != null) { //判断标注是否为距离标注和扯旗标注
                    for (var item in childMarkeds) {
                        top.LayerManagement.earth.DetachObject(childMarkeds[item]);
                    }
                } else {
    ...
    setMarkedComplex: function () {
        var thisImg = $("#MarkedComplex").find("img");
        Tools.cancelDisabled();
        disableAll(false);
        thisImg.attr("src", "images/tools/MarkedComplex.png");
    },
            ...
            if (urlList) {
                ...
                top.LayerManagement.earth.Event.OnEditDatabaseFinished = function (pRes, pFeature) {
                    ...
                    if (urlList.length == 0) {
                        ...
                        MarkedCreator.setMarkedComplex();
                    }
                    if (urlList.length != 0) {
                        MarkedCreator.sendMarkService(urlList, markText, complexPoints, visibility, dataParam, complexLineObj, w, p1, p2, divObj, markedName);
                    }
...
var elevationClick = function () {
    ...
    MarkedCreator.onMarkedCreate(1, function (resultData) {
        ...
        if (MarkedCreator.createElementIcon(dataParam1)) {
        ...
        if (MarkedCreator.createElementIcon(dataParam2)) {
...
var diameterClick = function (divObj) {
    ...
    MarkedCreator.onMarkedCreate(1, function (resultData) {
        ...
        var layerId = resultData.layerId;
        //var id = dataNode.selectSingleNode("US_ID").text;
        var diam = dataNode.selectSingleNode(us_size).text;
        // var width = parseFloat(dataNode.selectSingleNode("US_PWIDTH").text);
        // var height = parseFloat(dataNode.selectSingleNode("US_PHEIGHT").text);
        var startDeep = parseFloat(dataNode.selectSingleNode(us_sdeep).text);
        var endDeep = parseFloat(dataNode.selectSingleNode(us_edeep).text);
        ...
        if (!MarkedCreator.createElementIcon(dataParam)) {
            return;
        }
...
var coordinatesClick = function (divObj) {
    ...
    MarkedCreator.onMarkedCreate(1, function (resultData) {
        ...
        var startCoord = SYSTEMPARAMS.pipeDatum.des_BLH_to_src_xy(coordList[0].lon, coordList[0].lat, coordList[0].alt);
        var endCoord = SYSTEMPARAMS.pipeDatum.des_BLH_to_src_xy(coordList[1].lon, coordList[1].lat, coordList[1].alt);
        //坐标转换。结束
...
var customPartClick = function (divObj) {
    ...
    MarkedCreator.onMarkedCreate(1, function (resultData) {
        ...
        //打开标注属性选择列表对话框
        var markedList = openDialog("html/marked/customMarkedListDialog.html", param, 350, 330);
        if (markedList == null) {
            return;
        }
        ...
        for (var i = 0; i < markedList.length; i++) {
            ...
            var value = dataNode.selectSingleNode(markedAttr.attrName).text;
...
var slopeClick = function (divObj) {
    ...
    MarkedCreator.onMarkedCreate(1, function (resultData) {
        ...
        var datumPoint1 = SYSTEMPARAMS.pipeDatum.des_BLH_to_src_xy(coordList[0].lon, coordList[0].lat, coordList[0].alt);
        var datumPoint2 = SYSTEMPARAMS.pipeDatum.des_BLH_to_src_xy(coordList[1].lon, coordList[1].lat, coordList[1].alt);
        //坐标转换。结束
...
var curvedAngleClick = function (divObj) {
    ...
    MarkedCreator.onMarkedCreate(0, function (resultData) {
        ...
        var layerId = resultData.layerId;
        //"&c=d7c67beb-3c18-44d8-af4a-195ae4ef05e4_joint"
        var layerAry = layerId.split("_")[0];
        var layerGuid = layerAry.split("=")[1];
        var url = top.params.ip + "/pipeline?rt=angle&service=" + layerGuid + "&PointKey=" + key;
        //处理搜索结果
        top.LayerManagement.earth.Event.OnEditDatabaseFinished = function (pRes, pFeature) {
            if (pRes.ExcuteType == top.SystemSetting.excuteType) {
                var xmlStr = pRes.AttributeName;
...
function AngleAndLengthMark(divObj, pointTag) {
    ...
    //var markedNode = createElementNode("ElemenAngAndLenhtMarked",attrArr,MarkedCreator.markedDoc);
    top.LayerManagement.earth.Event.OnCreateGeometry = function (p, t) {
        ...
        if (v3s.Count === 2) {
            ...
            //top.LayerManagement.earth.GlobeObserver.FlytoLookat(dataParam.location.lon, dataParam.location.lat, dataParam.location.alt, 0, 90, 45, 45, 3);
            v3s.Remove(1);
        }
        pointTag = false;
...
function paramQuery(pFeat, guid, filter, queryType, queryTableType) {
