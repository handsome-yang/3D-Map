...
STAMP.TrackManager = function (earth) {
    var trackManager = {};
    var xmlTrackListFile = earth.RootPath + "\\track\\trackList";
     ...
     * @private
     */
    var _queryData = function (url) {
        var result = [];
        var xmlTrackData = "";
        if (url) {
            xmlTrackData = earth.UserDocument.LoadXmlFile(url + ".xml");
        } else {
            xmlTrackData = earth.UserDocument.LoadXmlFile(xmlTrackListFile + ".xml");
        }
    ...
    /**
     * 根据id创建track对象
     * @param {[string]} id     [要创建的track的guid]
     * @param {[string]} name   [track的名称]
     * @return {object}  创建出来的track对象
     */
     ...
     * @param dynamicId 动态物体GUID
     */
    var out = function (dynamicId) {//一定要先停止再卸载运动物体！！！
     ...
     */
    var startTracking = function (dynamicId, type) {
    ...
    trackManager._queryData = _queryData;
    trackManager.insertAfter = insertAfter;
    return trackManager;
};