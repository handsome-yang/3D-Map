 ...
 * 修改日期：2017年11月7日
 **************************************************/
var lastResult = false;//上一次是否有结果,为了防止mscrollbar报错
...
$(function () {
    ...
    /**
     * 显示高亮或者取消高亮
     * @param arr 传入的编号字符串数组
     * @param 上一个显示结果的图层的guid
     * @param pt 类型,line,point
     * @param isShow  是否需要高亮
     */
    ...
    $("#showResult").click(function () {
        ...
        clearHighLight();
        var checkTag = $('input:checkbox[name="showResult"]').is(":checked");//是否需要显示结果
        if (checkTag) {
            if (strArr.length) {//如果是之前已经有结果了则直接高亮了就行
                getHighLightObjects(strArr, guid, "line", true)
            } else {//如果之前没有结果那么需要先去查询再显示高亮
                earth.Event.OnEditDatabaseFinished = function (pRes) {
                    if (pRes.ExcuteType == top.SystemSetting.excuteType) {
                        var thisStr = "";
                        var xmlStr = pRes.AttributeName;
                        ...
                        if (result !== null) {
                            ...
                            var counts = records.length;
                            if (strArrLen == 0) {
                                strArrLen = Math.ceil(json.depthResult.num / 10000);//计算出一个会有多少个10000条记录
                            }
                            if (!counts) {//只有一个结果
                                records = [records];
                            }
                            for (var i = 0; i < records.length; i++) {
                                ...
                                if (deepIso > s_deep || deepIso > e_deep) {//起点或者终点埋深不符合标准,那么就需要在显示结果的时候高亮
                                    if (thisStr == "") {
                                        thisStr += fieldValue
                                    } else {
                                        thisStr += "," + fieldValue;
                            ...
                            strArr.push(thisStr);
                            strIndex++;
                            if (strIndex < strArrLen) {//如果还有没查询完，那么继续查
