 ...
 */
function record() {
    window.showModalDialog("animationSetting.html",params,"dialogWidth=220px;dialogHeight=150px;status=no");
    if(params){
...
function exportAnims(){
    ...
    var newNodes = zTree.addNodes(treeNode, {name:params.name,  pId:treeNode.pId, id:id,fps:params.fps, icon:animateIcon});
    if(treeNode.id != -1){
...
$(function () {
    var rightEId = "";//右键编辑的节点ID
    var xmlNode = "";//xml节点类型：folder、record等
    var TreeObj = null;//树节点
    var tag = "";//录制是否为新的
    var playTag = 1;//播放数量
    var recordNode = null;//播放的动画节点
    ...
    var clickNode = function (event,treeId,node){
        ...
        TreeObj.setting.callback.onRightClick = rightClickuserdataTreeNode;//开启右键
    ...
    $("#btnCreate").click(function (){
        ...
        if(Name){
            ...
            var treeNode = nodes[0];
            //从右键的节点作为父对象进行添加
            var newNodes = zTree.addNodes(treeNode, {name:Name, isParent:true, pId:treeNode.pId, id:guid,checked:true, icon: folderIcon});
