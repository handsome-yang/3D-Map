...
var tag_id="";//第几个div的ID
var idNumber = 1;//标示第几页
...
window.onload=function(){
    ...
    $("#import").click(function(){
        var fileXml2 = earth.UserDocument.LoadXmlFile(earth.RootPath + top.STAMP_config.constants.VIEWPOINTFILE + ".xml");
        ...
        if(fileDoc2.documentElement.firstChild.childNodes.length>0){
            setTimeout(function(){
                var fileXml1 = earth.UserDocument.LoadXmlFile(earth.RootPath + top.STAMP_config.constants.VIEWPOINTFILE + ".xml");
...
/**
 * 功能：IE10及以上浏览器不支持本地图片预览，故使用服务器上的试点图片
 * 返回值：服务器上试点图片所在路径
 */
var getWebView = function(){
    var path = top.STAMP_config.server.ip+"/viewImages/";
    return path;
}

/**
 * 节点添加图片
 * @param  {[num]} index [节点索引]
 * @param  {[object]} node  [节点]
 * @param  {[string]} tagId [目标节点的ID]
...
/**
 * 功能：创建视点节点
 * 参数：dataObj-视点创建参数，viewpointDoc-视点xml文档节点
 * 返回值：视点节点
 */
...
var getViewImageDir = function(){
    var dir = top.STAMP_config.server.ip.replace(/^http:\/{2}/g, '\\\\');
