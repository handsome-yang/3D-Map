...
var pipeLineLayers = top.SystemSetting.getPipeListByLayer(layer1);
setDivHeight();
...
$(function () {
    ...
    var InsertObject = function (obj) {
        ...
        var type = "line";
        var template = "<tr ondblclick=highlightObject('" + layerGuid + "','" + type + "','" + obj.GetKey() + "')><td class='chk'><input type='checkbox' name='chk' value='" + obj.GetKey() + "' checked></td><td class='col1'>" + obj.GetKey() + "</td><td class='col2'>管段</td><td class='col3'>" + layerName + "</td></tr>";
    ...
    function parseResult(result, layerId, type, customColor) {
        ...
        for (var i = 0; i < count; i++) {
            ...
            if (top.SYSTEMPARAMS.profileAlt == "0") {
                ...
                groundEndAlt = (parseFloat(groundEndAlt)).toFixed(2);
                pipeLineStartAlt = (parseFloat(groundStartAlt - endDeep)).toFixed(3); //管线开始高程
                pipeLineEndAlt = (parseFloat(groundEndAlt - startDeep)).toFixed(3);//管线结束高程
            }
    ...
    function createVerSectionChat() {
        ...
        $("#startAnalysis").attr("disabled",false);
        if (hasSvg()) {
            openDialog("../../html/pipelineAnalysis/verSectionResult.html", params, 1060, 700);
        } else {
            openDialog("../../html/pipelineAnalysis/verSectionResult2.html", params, 1060, 700);
        }
    }
});