...
$(function() {
     ...
     * @param  {[type]} layerName [图层名称]
     * @param  {[type]} type      [0：起点；1：终点]
     * @param  {[type]} tableName [展示的table div]
     * @param  {[type]} v3s       [点位坐标]
    ...
    var initLineValue = function(layerID, record, layerName) {
        ...
        deleteNullAndUndefine("tblLineResult");//删除空数据字段的显示
    };
    ...
    function initCustomValue(type, record, tab) {
        ...
        if (type == "line") {
            lineFieldMap = top.SYSTEMPARAMS.pipeFieldMap.getElementsByTagName("LineFieldInfo")[0];
        } else {
            lineFieldMap = top.SYSTEMPARAMS.pipeFieldMap.getElementsByTagName("PointFieldInfo")[0];
        }
        if (!lineFieldMap) {
            return;
        }
     ...
     * 获取平面坐标
     * @param  {[type]} layerID [图层ID]
     * @param  {[type]} data    [数据记录]
     * @param  {[type]} usKey   [管点编码值]
     * @return {[type]}         [description]
     */
     ...
     * @return {[type]}           [description]
     */
    var initRoomValue = function(layerID, record, layerName) {
    ...
    var bLine = (params.c.indexOf("container") > -1 || params.c.indexOf("powerline") > -1)?1:0;
    var filed = top.getName("US_KEY", bLine, true);
    var strPara = "(and,equal," + filed + "," + params.key + ")";
    var objParams = params.c.split("_");
    var layerID = objParams[0];
    var layer = earth.LayerManager.GetLayerByGUID(layerID);

    if (params.c && params.c.indexOf("room") != -1) { //判断是否是井室，井室单独查询
        ...
        earth.Event.OnEditDatabaseFinished = function(pRes, pFeature) {
            if (pRes.ExcuteType == top.SystemSetting.excuteType) {
                ...
                if (systemJson) {
                    $("#divPointResult").show();
                    $("#divLineResult").hide();
                    initRoomValue(layerID, systemJson.Result.Record, layer.Name);
                }
            }
        }
        earth.DatabaseManager.GetXml(strConn);
